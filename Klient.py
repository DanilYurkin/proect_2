...
class Add_Note_Windoww(QMainWindow):
    def __init__(self, email, name):
        ...
        # self.add_label.setText("Cамокат")
        ...
        # self.add_label.setText("Телефон")
        # self.add_label.setStyleSheet("font-size:24px")
        #
        # self.text_input = QPlainTextEdit(self)
        # self.text_input.move(250, 80)
        # self.text_input.resize(100, 300)
        # self.text_input.setPlaceholderText("Телефон")
        # self.text_input.setStyleSheet(
        #     "background:#ffffff; border:1px solid #a9a9a9;border-radius:3px;font-size:16px;")
        # self.text_input.setFocus()
        #
        ...
        # self.add_label.setText("ФИО")
        # self.add_label.setStyleSheet("font-size:24px")
        #
        # self.text_input = QPlainTextEdit(self)
        # self.text_input.move(100, 80)
        # self.text_input.resize(100, 300)
        # self.text_input.setPlaceholderText("ФИО")
        # self.text_input.setStyleSheet(
        #     "background:#ffffff; border:1px solid #a9a9a9;border-radius:3px;font-size:16px;")
        # self.text_input.setFocus()
        ...
        self.tableWidget.setGeometry(25, 50, 400, 400)
        ...
        try:
            ...
            cursor.execute(f"SELECT id_polzovatel, telefon, name FROM polzovatel;")
            records = cursor.fetchall()
            print(len(records))
            for a in range(math.ceil(len(records) / 2) + 1):
                self.tableWidget.insertRow(a)
                if a > 0:
                    if a - 1 < len(records):
                        record_1 = records[a - 1]
                        for col_index, value in enumerate(record_1[:3]):
            ...
            self.tableWidget.setSpan(0, 1, 0, 3)

            new_item = QTableWidgetItem("ID_Клиента")
            self.tableWidget.setItem(0, 0, new_item)
            new_item = QTableWidgetItem("Телефон")
            self.tableWidget.setItem(0, 1, new_item)
            new_item = QTableWidgetItem("Имя")
